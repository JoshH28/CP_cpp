...
ll dfs(ll node, ll par){
    ll leafcount = 0;
    ll bees = freq[node], flag=0;
    for (auto it: adjlist[node]) {
        if (it == par) continue;
        leafcount += dfs(it, node);
        flag=1;
    }
    if (flag == 0) leafcount = 1;
    ans += min(bees, leafcount);
    leafcount = max(0ll, leafcount - bees);
    return leafcount;
}
...
void solve() {
    ...
    dfs2(1, -1); //cout << findleaves << "\n";
    for (ll q = 0; q < findleaves; q++) {
        freq[wow[q]]++;
    }
    dfs(1, -1); cout << ans;
