...
#pragma GCC target("avx2,fma,bmi,bmi2,abm,lzcnt,popcnt,tune=native")
...
void dfs(ll node, ll par) {
   ll sigma = pos[arr[node]];
   ll currval = arr[node], prevval = root->range_sum(sigma, sigma);
   ll wow = max(root->range_sum(sigma, sigma), 1+ root->range_max(0, sigma-1));
   ans[node] = wow;
   root->set(sigma, sigma, wow);
   for (auto it: adjlist[node]) {
      if (it == par) continue;
      dfs(it, node);
      root->set(sigma, sigma, wow);
   }
   root->set(sigma, sigma, prevval);
}

void finalcalc(ll node, ll par, ll currmax) {
   ans[node] = max(currmax, ans[node]);
   for (auto it: adjlist[node]) {
      if (it==par) continue;
      finalcalc(it, node, ans[node]);
   }
}
...
void solve() {
    ...
    dfs(1, -1); finalcalc(1, -1, -1);
    for (ll q = 1; q <= n; q++) cout << ans[q] << "\n";
}
