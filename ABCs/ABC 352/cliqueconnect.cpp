...
void solve() {
    ll n, m, k, cost; cin >> n >> m;
    for (ll q = 0; q <= n+10; q++) {
        p[q] = q;
    }
    for (ll q = 0; q < m; q++) {
        cin >> k >> cost; ll indeed[k];
        for (ll q = 0; q < k; q++) cin >> indeed[q];
        sort(indeed, indeed+k);
        for (ll q = 0; q < k-1; q++) {
            edge temp(indeed[q], indeed[q+1], cost);
            edgeList.pub(temp);
        }
    }
