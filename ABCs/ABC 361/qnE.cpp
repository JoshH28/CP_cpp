...
void solve() {
    ...
    ll end1 = 1;
    for (ll q = 1; q <= n; q++) {
        if (dist[q] > dist[end1]) end1 = q;
    }
    memset(dist, -1, sizeof(dist));
    dist[end1] = 0;
    pq.push(make_pair(0, end1)); // for multisource just push more nodes with dist 0
    while (!pq.empty()) {
       ll cn = pq.top().second, cd = pq.top().first;
       pq.pop();
       if (cd!=dist[cn]) continue;
    ...
    ll diameter = 0;
    for (ll q = 1; q <= n; q++) {
        diameter = max(dist[q], diameter);
    }
    ll ans = edgesum*2 - diameter;
    cout << ans;
