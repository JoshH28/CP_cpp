...
void solve() {
    ll n, m; cin >> n >> m; 
    ll weight[n+10], dist[n+10];
    vector<pll> adjlist[n+10];
    memset(dist, -1, sizeof(dist));
    for (ll q = 1; q <= n; q++) cin >> weight[q];
    ll t1, t2, t3; dist[1] = weight[1];
    for (ll q = 0; q < m; q++) {
        cin >> t1 >> t2 >> t3;
    ...
    pq.push(make_pair(dist[1], 1)); // for multisource just push more nodes with dist 0
    while (!pq.empty()) {
       ll cn = pq.top().second, cd = pq.top().first;
       pq.pop();
       if (cd!=dist[cn]) continue;
       for (auto it: adjlist[cn]) {
           ll nx = it.first, nd = cd+it.second+weight[nx]; // change nd to max/min(d, it->second)​ for max and min flow. 
