...
void solve() {
   root = new node(0, 2*1e9+1);
   ll n; cin >> n; ll a1[n], a2[n];
   for (ll q = 0; q < n; q++) cin >> a1[q];
   for (ll q = 0; q < n; q++) cin >> a2[q];
   for (ll q = 0; q < n; q++) root->set(a1[q]+1e9, a1[q]+1e9, a2[q]);
   ll qn, l, r; cin >> qn;
   for (ll q = 0; q < qn; q++) {
      cin >> l >> r;
      cout << root->range_sum(l+1e9, r+1e9) << "\n";
   }
