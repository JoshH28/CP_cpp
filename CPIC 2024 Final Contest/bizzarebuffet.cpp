...
void solve() {
    ll n; cin >> n; ll arr[n+10]; ll dp[n+10];
    for (ll q = 1; q <= n; q++) cin >> arr[q];
    node* neg = new node(0, n+10);
    node* pos = new node(0, n+10);
    memset(dp, 0, sizeof(dp)); dp[0] = 0; dp[1] = 0; 
    neg->set(1, 1, dp[1] - arr[1]); pos->set(1, 1, dp[1] + arr[1]);
    for (ll q = 2; q <= n; q++) {
        ll minus = neg->range_max(1, q-1);
        ll plus = pos->range_max(1, q-1);
        dp[q] = max(minus + arr[q], plus - arr[q]);
        neg->set(q, q, dp[q] - arr[q]);
        pos->set(q, q, dp[q] + arr[q]);
    }
    cout << dp[n];
}
