...
void solve() {
    ...
    for (ll q = 0; q < arr.size(); q++) {
        if (mp[arr[q]] % 2 == 1) flag=0;
    }
    if (flag == 1) {
        cout << "YES\n"; return;
    }
    sort(arr.begin(), arr.end());
    ll currallowance = (mp[arr[0]]-2);
    if (mp[arr[0]]==1) {
        cout << "NO\n"; return;
    }
    for (ll q = 1; q < arr.size(); q++) {
        if (arr[q-1] + 1 + currallowance/2 >= arr[q]) {
            currallowance -= (arr[q] - arr[q-1])*2;
            currallowance += mp[arr[q]] - 2;
        } else {
            cout << "NO\n"; return;
        }
    }
    if (currallowance%2 ) cout << "NO\n";
    else cout << "YES\n";
