...
ll memo(ll index) {
    ...
    ll ans = INT_MAX, maxn=arr[index]; 
    for (ll q = index; q < n; q++) {
        maxn = max(arr[q], maxn);
        ans = min(ans, maxn*(q-index+1) + memo(q+1));
    }
    return dp[index] = ans;
}
...
void solve() {
    cin >> n; ll maxn = -INT_MAX; 
    for (ll q = 0; q < n; q++) {
        cin >> arr[q]; maxn = max(maxn, arr[q]);
    }
    ll ans = INT_MAX;
    if (n == 1) cout << arr[0];
    else if (n == 2) cout << min({arr[0] + arr[0]+ arr[1], 2*max(arr[0], arr[1])});
    else {
    ll ans = min({n*maxn, 3*arr[0] + 2*max(arr[1], arr[2]), arr[0]*3 + arr[1]*2 + arr[2]});
    ans = min(ans, 3*max(arr[0], arr[1]) + arr[2]); 
    cout << ans;
    }
