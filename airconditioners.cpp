...
void solve() {
    ...
    ll ti, ai; pll ac[k+10]; ll ans[n+10];
    for (ll q = 1; q <= n; q++) {
        left->set(q, q, INT_MAX/6);
        right->set(q, q, INT_MAX/6);
    }
    for (ll q = 0; q < k; q++) cin >> ac[q].fi;
    for (ll q = 0; q < k; q++) {
        cin >> ac[q].se;
        left->set(ac[q].fi, ac[q].fi, ac[q].se - ac[q].fi);
        right->set(ac[q].fi, ac[q].fi, ac[q].se + ac[q].fi);
    }
    for (ll q = 1; q <= n; q++) {
        ans[q] = min(left->range_min(1, q) + q, right->range_min(q, n) - q);
        //cout << left->range_min(1, q)+q << " " << right->range_min(q, n) - q << " ";
        cout << ans[q] << " "; 
    }
    cout << "\n";
}
