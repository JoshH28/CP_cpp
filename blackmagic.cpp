...
ll dn, spelln, ans = INT_MAX;
pair<pll, ll> defences[101];
pair<pll, pll> spells[101];



void dp(ll n, vector<ll> currsubsets) {
  if (n == spelln) return;
  dp(n+1, currsubsets);
  currsubsets.pub(n);
  subsets.pub(currsubsets);
  dp(n+1, currsubsets);
}
...
void solve() {
  cin >> dn >> spelln;
  for (ll q = 0; q < dn; q++) {
    cin >> defences[q].first.first >> defences[q].first.second >> defences[q].second;
  }
  for (ll q = 0; q < spelln; q++) {
    cin >> spells[q].first.first >> spells[q].first.second >> spells[q].second.first >> spells[q].second.second;
  }
  vector<ll> empty;
  dp(0, empty);
  for (auto it1: subsets) {
    ...
    root = new node(0, 101);
    for (auto it2: it1) {
      cost += spells[it2].second.second;
      root->add(spells[it2].first.first, spells[it2].first.second, spells[it2].second.first);
    }
    for (ll q = 0; q < dn; q++) {
      ll query = root->range_min(defences[q].first.first, defences[q].first.second);
      if (query < defences[q].second) {
        flag = 0; break;
      }
    }
    if (flag) {
