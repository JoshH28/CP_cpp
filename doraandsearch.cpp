...
void solve() {
    ...
    set<ll> yes;
    for (ll q = 0; q < n; q++) {
        cin >> arr[q]; yes.insert(arr[q]);
    }
    ll l = 0, r = n-1;
    while (l <= r) {
        while (l <= r && (arr[l] == *yes.begin() || arr[l] == *yes.rbegin())) {
            yes.erase(arr[l]); l++;
        } 
        while (l <= r && (arr[r] == *yes.begin() || arr[r] == *yes.rbegin())) {
            yes.erase(arr[r]); r--;
        }
        if (l > r) break;
        if ((arr[l] != *yes.begin() && arr[l] != *yes.rbegin()) && (arr[r] != *yes.begin() && arr[r] != *yes.rbegin())) {
            cout << l+1 << " " << r+1 << "\n"; return;
        }
    }
    cout << "-1\n";
