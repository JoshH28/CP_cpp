...
ll arr[200001];
pair<ll, pll> ans[200001]; 
pair<pll, ll> queries[200001];
...
void solve() {
    ...
    for (ll q = 0; q < qn; q++) {
        ...
        queries[q].first.first -= 1;
        queries[q].first.second -= 1;
        queries[q].second = q;
    } 
    memset(mp, 0, sizeof(mp));
    sort(queries, queries+qn, cmp);
    ll cnt = 0, lptr = 1, rptr = 0;
    for (ll q = 0; q < qn; q++) {
        ll start = queries[q].first.first, end = queries[q].first.second;
        while(lptr > start) {
            lptr--;
            mp[arr[lptr]]++;
        ...
        while(rptr < end) {
            rptr++;
            if (mp[arr[rptr]] == 0) cnt++;
            mp[arr[rptr]]++;
        }
        while (lptr < start) {
            mp[arr[lptr]]--;
            if (mp[arr[lptr]] == 0) cnt--;
            lptr++;
        }
        while (rptr > end) {
            mp[arr[rptr]]--;
        ...
        //cout << cnt << " " << lptr << " " << rptr << " " << queries[q].second << "\n";
        ans[queries[q].second] = {cnt, {lptr, rptr}};
    }
    for (ll q = 0; q < qn; q++) cout << ans[q].first << "\n";
}
