...
struct DSU {
    ...
    int par(int x) {
        if (parent[x] == x) {
            ...
            int pathCompression = par(parent[x]);
...
void solve() {
    ...
    DSU *graph1 = new DSU(n);
    DSU *graph2 = new DSU(n);
    int ans = 0;
    vector<pll> edgef;
    for (ll q = 0; q < m1; q++) {
        int one, two; cin >> one >> two;
        edgef.pub({one, two});
        //graph1->merge({one, two});
    }
    for (int q = 0; q < m2; q++) {
        int one, two; cin >> one >> two;
        graph2->merge(one, two);
    }
    for (auto it: edgef) {
        if (!graph2->isSameSet(it.fi, it.se)) ans++;
        else graph1->merge(it.fi, it.se);
    }
    cout << ans + graph1->noOfComponents - graph2->noOfComponents << "\n";
