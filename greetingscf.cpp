//Take your pick, take your time, son, because this is you lighting the world on fire
#include <bits/stdc++.h>
using namespace std;
#define fi first
#define se second
...
void solve() {
    ...
    for (ll q = 0; q < gay.size(); q++) {
        idx[gay[q]] = q+1;
    }
    sort(arr, arr+n, cmp);
    for (ll q = 0; q < n; q++) {
        ans += root->range_sum(0, idx[arr[q].fi]);
        root->set(idx[arr[q].fi], idx[arr[q].fi], 1);
    }
    cout << ans << "\n";
}
