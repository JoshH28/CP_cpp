...
void solve() {
    ll n, k; cin >> n >> k; 
    ll arr[n+10], psum[n+10]; psum[0] = 0;
    for (ll q = 1; q <= n; q++) {cin >> arr[q]; psum[q] = psum[q-1] + arr[q];}
    if (k == 0) {
        ll best = 0, local = 0;
        for (ll q = 1; q <= n; q++) {
            if (arr[q]) {
                ...
                best = max(local, best); local = 0;
            }
        }
        best = max(local, best);
        cout << best << "\n";
        for (ll q = 1; q <= n; q++) {cout << arr[q] << " ";}
        return;
    }
    ll maxlength = 0, lo = 0, hi = n;
    while (lo<=hi) {
        ll mid = (lo+hi)/2, localmin = INT_MAX;
        for (ll q = 0; q <= n-mid+1; q++) {
            ll bruh = mid - (psum[q+mid-1] - psum[q-1]);
            localmin = min(localmin, bruh);
        }
        if (localmin <= k) {
    ...
    cout << maxlength << "\n";
    ll flag = 0, cnt = 0;
    for (ll q = 1; q <= n-maxlength+1; q++) {
        ll bruh = maxlength - (psum[q+maxlength-1] - psum[q-1]);
        if (cnt!=0) {
            cout << 1 << " "; cnt--;
        } else if (flag == 1) {
            cout << arr[q] << " " ;
        } else {
            if (bruh <= k && k != 0) {cout << 1 << " "; cnt = maxlength-1; flag = 1;}
            else cout << arr[q] << " ";
        }
    }
    for (ll q = n-maxlength+2; q <= n; q++) {
        if (cnt!=0) {
            cout << 1 << " "; cnt--;
        } else if (flag == 1) {
            cout << arr[q] << " " ;
