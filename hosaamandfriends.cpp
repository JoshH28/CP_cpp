...
void solve() {
    ll n, m, ans=0, a, b, currright = n; cin >> n >> m; pll blocks[m];
    ll friends[n+10]; for (ll q = 0; q < n+10; q++) friends[q] = n;
    for (ll q = 0; q < m; q++) {
        cin >> a >> b;  if (a > b) swap(a, b);
        friends[a] = min(friends[a], b-1);
    }
    for (ll q = n-1; q >= 0; q--) friends[q] = min(friends[q+1], friends[q]);
    for (ll q = 1; q <= n; q++) {
      ans += (friends[q] - q);
    }
    cout << ans+n << "\n";
