...
// ll recur(ll curr) {
//   if (curr == n) return arr[n]!=x;
//   if (dp[curr] != -1) return dp[curr];
//   ll wow = psum[curr] + x;
//   ll maxindex = upper_bound(psum.begin(), psum.end(), wow)-psum.begin();
//   if (maxindex >= n) {
//     dp[curr] =  n - curr + 1;
//   }
//   else {
//     dp[curr] =maxindex - curr + recur(maxindex);
//   }
//   return dp[curr];
// }
...
void solve() {
  ...
  ll ans = 0;
  for (ll q = n; q > 0; q--) {
    ll wow = upper_bound(arr, arr+n+1, arr[q-1] + x)-arr;
    dp[q] = dp[wow+1] + wow - q;
  }
  for (ll q = 1; q <= n; q++) ans += dp[q];
  cout << ans << "\n";
}
