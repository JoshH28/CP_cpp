...
//             } else {
//                 return min(l->qry(left, mid), r->qry(mid+1, right));
//             }
//         }
//     }
// } *root;
...
void solve() {
    ...
    for (ll q = 1; q < m; q++) {
        ll longway, shortway;
        if (visits[q] > visits[q-1]) {
            shortway = visits[q]-visits[q-1];
            longway = n - visits[q] + visits[q-1];
            if (longway > shortway) {
                root->add(visits[q], n, longway-shortway);
                root->add(1, visits[q-1], longway-shortway);  
            } else {
                root->add(visits[q-1], visits[q], shortway-longway);
            }
        } else {
            shortway = visits[q-1] - visits[q];
            longway = n - visits[q-1] + visits[q];
            if (longway > shortway) {
                root->add(visits[q-1], n, longway-shortway);
                root->add(1, visits[q], longway-shortway);  
            } else {
                root->add(visits[q], visits[q-1], shortway-longway);
            }
        }
    }
    ll lol = INT_MAX, cnt = 0;
    lol = root->range_min(1, n);
    for (ll q = 1; q < m; q++) {
        ll longway, shortway;
        if (visits[q] > visits[q-1]) {
            shortway = visits[q]-visits[q-1];
