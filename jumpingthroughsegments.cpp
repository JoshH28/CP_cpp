...
void solve() {
    ll n, lo = 0, hi = 1e9, ans = INT_MAX; cin >> n; pll segs[n];
    for (ll q = 0; q < n; q++) cin >> segs[q].first >> segs[q].second;
    while (lo <= hi) {
        ll mid = (lo+hi)/2, currleft = 0, currright = mid, flag=1; // we are trying this value of jump;
        for (ll q = 0; q < n; q++) {
            if (segs[q].second < currleft || segs[q].first > currright) {flag=0; break;}
            currleft = max(currleft, segs[q].first);
            currleft -= mid;
            currleft = max(currleft, 0ll);
            currright = min(currright, segs[q].second);
            currright += mid;
        }
        if (flag) {// can, try lower;
