...
void solve() {
    ll n, a, b, ans = INT_MAX, sum = 0; cin >> n >> a >> b; 
    ll arr[n+10], psum[n+10];  memset(psum, 0, sizeof(psum));
    for (ll q = 1; q <= n; q++) {
        cin >> arr[q];
        psum[q] = psum[q-1] + arr[q]; 
    } 
    arr[0] = 0; ll acnt = 0;
    ans = min(ans, b*(psum[n] - psum[0]));
    for (ll q = 1; q <= n; q++) {
        acnt += a*(arr[q]-arr[q-1]); // cost to move capital to city q
        ans = min(ans, acnt + b*(psum[n] - psum[q] - arr[q]*(n - q - 1)));
    }
    cout << ans << "\n";
}
