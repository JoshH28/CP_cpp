...
void solve() {
    ...
    root = new node(0, n-1);
    for (ll q = 0; q < n; q++) {
        cin >> a[q]; root->set(q, q, a[q]);
    }
    for (ll q = 0; q < m; q++) {
        cin >> b[q];
        ll ballno = root->range_sum(b[q], b[q]);
        root->set(b[q], b[q], 0);
        if (ballno >= n) {
            root->add(0, n-1, ballno/n);
        }
        ballno%=n;
        if (ballno + b[q] >= n) {
            if (b[q] +1 < n) root->add(b[q]+1, n-1, 1);
            root->add(0, (ballno+b[q])%n ,1);
        } else {
            root->add(b[q]+1, b[q]+ballno, 1);
        }
    }
    for (ll q = 0; q < n; q++) {
        cout << root->range_sum(q, q) << " ";
    }
