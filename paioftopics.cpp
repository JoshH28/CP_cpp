...
void solve() {
   ll n; cin >> n; ll a[n], b[n], c[n];
   for (ll q = 0; q < n; q++) cin >> a[q];
   for (ll q = 0; q < n; q++) cin >> b[q];
   root = new node(0, n);
   pll idxcomp[n]; unordered_map<ll,ll> mp;
   for (ll q = 0; q < n; q++) {
     idxcomp[q] = {a[q] - b[q], q};
     c[q] = a[q] - b[q];
     //root->add(c[q], c[q], 1);
   }
   sort(idxcomp, idxcomp+n); sort(c, c+n);
   for (ll q = 0; q < n; q++) {
      mp[idxcomp[q].second] = q;
      root->add(q, q, 1); 
   }
   ll ans = 0;
   for (ll q = 0; q < n; q++) {
      root->add(mp[q], mp[q], -1);
      ll up = upper_bound(c, c+n, -(a[q] - b[q]) )-c;
      ans += (root->range_sum(up, n));
      //cout << root->range_sum(up, n) << " ";
   }
   cout << ans;
}
