...
ll n, m, k;
ll vis[MAXN];
vector<ll>newcomponent;

ll dfs(ll node, ll rootnode) {
    newcomponent.pub(node);
    vis[node] = 1;
    ll localans = 1;
    for (auto it: adj[node]) {
        if (vis[it] == 1) continue;
        if (beef[rootnode].find(it) != beef[rootnode].end()) continue;
        localans += dfs(it, rootnode);
    }
    return localans;
}
...
void solve() {
    ...
    for (ll q = 0; q < m; q++) {
        ll t1, t2; cin >> t1 >> t2;
        adj[t1].pub(t2); adj[t2].pub(t1);
    }
    cin >> k;
    for (ll q = 0; q < k; q++) {
        ll t1, t2; cin >> t1 >> t2;
        beef[t1].insert(t2); beef[t2].insert(t1);
    }
    ll bestans=0;
    for (ll q = 1; q <= n; q++) {
        if (vis[q]) continue;
        newcomponent.clear();
        ll localans = dfs(q, q), flag=1;
        for (auto it: newcomponent) {
            for (auto it2: newcomponent) {
                if (it == it2) continue;
                if (beef[it].find(it2) != beef[it].end()) flag=0;
            }
        }
        if (bestans < localans && flag) {
            bestans = max(bestans, localans);
            //cout << q << " " << localans << "\n";
        }
    }
    cout << bestans;
