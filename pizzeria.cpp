...
void solve() {
    ...
    node *root2 = new node(0, n+1);
    for (ll q = 1; q <= n; q++) {
        cin >> t1; 
        root->set(q, q, t1-q); root2->set(q,q,t1+q);
    }
    ...
    for (ll q = 0; q < qn; q++) {
        ...
        if (t1 == 1) {
            cin >> t2 >> t3;
            root->set(t2, t2, t3-t2); root2->set(t2, t2, t3+t2);
      
        } else if (t1 == 2) {
            cin >> t2;
            ll yes = root->range_min(1, t2)+t2;
            ll hmm =  root2->range_min(t2, n)-t2;
            //cout << yes << " " << hmm << " ";
            yes = min(hmm, yes);
            cout << yes << "\n";
        }
