...
void solve() {
    ll n, currt=0, ans=1; cin >> n; pll arr[n];
    unordered_set<ll> lol;
    for (ll q = 0; q < n; q++) {
        cin >> arr[q].first >> arr[q].second;
        arr[q].second += arr[q].first;
    }
    sort(arr, arr+n);
    // cout << "\n";
    // for (ll q = 0; q < n; q++) {
    ...
    currt = arr[0].first+1;
    for (ll q = 1; q < n; q++) {
        //if (arr[q].second > 1e18) break;
        if (currt < arr[q].first) currt = arr[q].first;
        if (currt > arr[q].second) continue;
        currt++; ans++;
