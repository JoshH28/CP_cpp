...
void solve() {
   ll n; cin >> n; ll arr[n+10], regret[n+10], cnt=1;
   for (ll q = 1; q <= n; q++) {cin >> arr[q]; regret[q] = arr[q];}
   map<ll, ll> why; set<ll> gay; sort(regret+1, regret+n+1);
   for (ll q = 1; q <= n; q++) gay.insert(regret[q]);
   for (auto it: gay) {why[it] = cnt; cnt++;}
   root = new node(0, cnt + 10);
   for (ll q = 1; q <= n; q++) {
      root->set(why[arr[q]], why[arr[q]], q);
   }
   for (ll q = 1; q <= n; q++) {
      ll yes = root->range_max(0, why[arr[q]]-1); 
      //cout << yes << " ";
      if (yes <= 0) {cout << -1 << " ";}
      else {
         if (yes - q - 1 < 0) {
