...
void solve() {
    ll n, k, t1, t2, t3; cin >> n >> k;
    vector<ll> both, alice, bob;
    for (ll q = 0; q < n; q++) {
        cin >> t1 >> t2 >> t3;
        if (t2 == 1 && t3 == 1) both.pub(t1);
        else if (t2 == 1) alice.pub(t1);
        else if (t3 == 1) bob.pub(t1); 
    }
    node a(0, alice.size()+5), two(0, both.size()+5), b(0, bob.size()+5);
    sort(both.begin(), both.end());
    sort(alice.begin(), alice.end());
    sort(bob.begin(), bob.end());
    ll maxlength = min(k, ll(both.size())), finalans = INT_MAX; 
    for (ll q = 0; q < alice.size(); q++) a.set(q+1, q+1, alice[q]);
    for (ll q = 0; q < bob.size(); q++) b.set(q+1, q+1, bob[q]);
    for (ll q = 0; q < both.size(); q++) two.set(q+1, q+1, both[q]);
    b.set(0, 0, 0); a.set(0, 0, 0); two.set(0, 0, 0);
    //cout << "\n";
    //cout << maxlength << " " << alice.size() << " " << bob.size() << " " << both.size() << "\n";
    for (ll q = 0; q <= maxlength; q++) {
        //cout << k-q << " ";
        if (k - q > alice.size() || k - q > bob.size()) continue;
        else {
            //cout << two.range_sum(1, q);
            //cout << " " << b.range_sum(1, k-q);
            //cout << " " << a.range_sum(1, k-q) << "\n";
            finalans = min(finalans, two.range_sum(0, q) + b.range_sum(0, k-q) + a.range_sum(0, k-q));
        }
    }
    // for (ll q = 0; q < maxlength; q++) {
    //     bothcnt += both[q];
    // }
    // if (k - (maxlength) >= alice.size() || k - (maxlength) >= bob.size()) {}
    // else {
    //     for (ll q = 0; q < k - (maxlength); q++) {
    //         alicecnt+=alice[q]; bobcnt += bob[q]; ci++; 
    //     }
    //     finalans = min(finalans, alicecnt+bobcnt+bothcnt);
    // }
    // bothcnt-=both[maxlength-1];
    // for (ll q = maxlength-2; q >= 0; q--) {
    //     if (k - (q+1) >= alice.size() || k - (q+1) >= bob.size()) {}
    //     else {
    //         alicecnt+=alice[ci]; bobcnt+=bob[ci];
    //         finalans = min(finalans, alicecnt+bobcnt+bothcnt);
    //     }
    //     bothcnt -= both[q];
    // }
    if (finalans == INT_MAX) cout << -1;
    else cout << finalans;
}
