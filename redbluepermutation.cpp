...
void solve() {
    ll n; cin >> n; ll t1; char t2;
    vector<ll> dec, inc, arr(n);
    for (ll q = 0; q < n; q++) cin >> arr[q];
    string gay; cin >> gay;
    for (ll q = 0; q < n; q++) {
        t2 = gay[q];
        if (t2 == 'B') dec.pub(arr[q]);
        else inc.pub(arr[q]);
    }
    sort(dec.begin(), dec.end());
    sort(inc.begin(), inc.end());
    reverse(inc.begin(), inc.end());
    for (ll q = 0; q < dec.size(); q++) {
        if (dec[q] >= q+1) continue;
        else {
            cout << "No\n"; return;
        } 
    }
    for (ll q = 0; q < inc.size(); q++) {
        if (inc[q] <= n-q) continue;
        else {
            cout << "No\n"; return;
        } 
    }
