...
void solve() {
    ...
    gay = new DSU(27);
    unordered_set<ll> touched;
    for (ll q = 0; q < n; q++) {
        cin >> str; ll bruh = ll(str[0] - 'a');
        touched.insert(bruh);
        for (ll q = 0; q < str.length(); q++) {
            gay->merge(bruh, ll(str[q] - 'a'));
            //cout << str[q] << " " << char(gay->parent[ll(str[q] - 'a')]+ll('a')) << "\n";
            touched.insert(ll(str[q] - 'a'));
        }
    }
    //cout << "\n";
    unordered_set<ll> bruh;
    for (ll q = 0; q <= ll('z' - 'a'); q++) {
        if (touched.find(q) == touched.end()) continue;
        //cout << char(q + ll('a')) << " " << char(gay->parent[q] + ll('a')) << "\n";
        bruh.insert(gay->par(q));
    }
    cout << bruh.size();
}
