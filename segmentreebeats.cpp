...
void solve() {
    ll n, qn, type, sus, gay; cin >> n >> qn; string nums[n+1]; 
    for (ll q = 1; q <= n; q++) cin >> nums[q];
    root = new node(1, n);
    for (ll q = 1; q <=n; q++) {
        root->set(q, q, 0);
    }
    for (ll q = 0; q < qn; q++) {
        ...
        if (type == 1) {
            ...
            cin >> sus;
            if (nums[sus].length() == 1) {
                cout << nums[sus] << "\n"; continue;
            }
            ll optimes = root->range_max(sus, sus), hold=0;
            //cout << optimes << " ";
            for (ll w = 0; w < optimes; w++) {
                //cout << nums[sus] << " ";
                hold = 0;
                if (nums[sus].length() == 1) {
                    break;
                }
                for (ll r = 0; r < nums[sus].length(); r++) {
                    //cout << nums[sus][r];
                    hold = ll(nums[sus][r] - '0') + hold;
                }
                //cout << hold;
                nums[sus] = to_string(hold);
                //cout << " ";
            }
            cout << nums[sus] << "\n";
            root->set(sus, sus, 0);
