...
struct node
{
    ...
    {
        if (start == __start && __end == end)
        {
            return value;
        }
        if (__end <= mid)
        {
            return l->rangeQuery(__start, __end);
        }
        else if (__start > mid)
        {
            return r->rangeQuery(__start, __end);
        }
        else
        {
            return min(l->rangeQuery(__start, mid), r->rangeQuery(mid + 1, __end));
        }
    }
} *root;
