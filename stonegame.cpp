...
ll n,maxn=0, minn = 0;
    ...
    for (ll q = 0; q < n; q++) {
        cin >> arr[q]; 
        if (arr[maxn] < arr[q]) maxn = q;
        if (arr[minn] > arr[q]) minn = q;
    }
    //cout << minn << " " << maxn << " ";
    ll ans = max(maxn, minn)+1;
    ans = min(ans, n - min(maxn, minn));
    if (maxn < minn) {
        ll temp = maxn; maxn = minn; minn = temp;
    }
    
    ans = min(ans, minn+1 + n - maxn);
    cout << ans << "\n";
}
