...
void solve() {
    ll n, query, x; cin >> n;
    root = new node(0, n);
    priority_queue<ll> types[n+1];
    vector<ll> takeOrNot;
    for (ll q = 1; q <= n; q++) {
        cin >> query >> x;
        if (query == 1) {
            types[x].push(takeOrNot.size());
            takeOrNot.pub(0);
        } else {
            if (types[x].empty()) {
                cout << -1; return;
            }
            ll prevPos = types[x].top();
            types[x].pop();
            root->add(prevPos, takeOrNot.size()-1, 1);
            takeOrNot[prevPos] = 1;
        }
    }
    cout << root->range_max(0, takeOrNot.size()-1) << "\n";
    for (auto it: takeOrNot) {
        cout << it << " ";
    }
}
