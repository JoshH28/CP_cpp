...
void solve() {
    ll n; cin >> n;
    ll cap[n+10], tea[n+10], ans[n+10];
    ll psum[n+10]; psum[0] = 0;
    memset(ans, 0, sizeof(ans));
    root = new node(0, n+10);
    for (ll q = 1; q <=n ; q++) cin >> tea[q]; 
    for (ll q = 1; q <= n; q++) {
        cin >> cap[q];
        psum[q] = psum[q-1] + cap[q];
    }
    for (ll q = 1; q <= n; q++) {
        ll wow = psum[q-1] + tea[q];
        ll astounding = upper_bound(psum, psum+n+1, wow) - psum;
        if (astounding > q) {
            root->add(q, astounding-1, 1);
        }
        ans[astounding] += (tea[q] + psum[q-1] - psum[astounding-1]);
    }
    for (ll q = 1; q<=n; q++) {
        ll finalans = cap[q]*root->range_sum(q, q) + ans[q];
        cout << finalans << ' ';
    }
    cout << "\n";
}
