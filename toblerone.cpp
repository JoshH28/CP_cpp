...
void solve() {
    ...
    ll dp[n]; dp[0] = max(0ll, arr[0]);
    for (ll q = 1; q < n; q++) {dp[q] = max(arr[q], arr[q] + dp[q-1]);}
    root = new node(0, n+10);
    for (ll q = 0; q < n; q++) root->set(q, q, dp[q]);
    ll maxans = 0; //root->set(n, n, -INT_MAX/10);
    for (ll q = 0; q < n; q++) {
        maxans = max(maxans, root->range_max(0, q) + root->range_max(q+1, n));
        cout << root->range_max(0, q) << " " << root->range_max(q+1, n) << "\n";
    }
    ll kadane[n]; kadane[0] = min(0ll, arr[0]);
    for (ll q = 1; q < n; q++) {kadane[q] = min(arr[q], kadane[q] + kadane[q-1]);}
    ll minans = 0;
    for (ll q = 0; q < n; q++) root->set(q, q, kadane[q]);
    root->set(n, n, INT_MAX / 10);
    for (ll q = 0; q < n; q++) {
        minans = min(minans, root->range_min(0, q) + root->range_min(q+1, n));
    }
    ll s = 0; for (ll q = 0; q < n; q++) s += arr[q];
    cout << max(s - minans, maxans);
}
