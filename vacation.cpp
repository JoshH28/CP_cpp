...
ll recur(ll index, ll prev)
{
    ...
    if (prev != 0)
    {
        ans = max(ans, a[index] + recur(index + 1, 0ll));
    }
    if (prev != 1)
    {
        ans = max(ans, b[index] + recur(index + 1, 1ll));
    }
    if (prev != 2)
    {
        ans = max(ans, c[index] + recur(index + 1, 2ll));
    }
    dp[index][prev] = ans;
    return ans;
}
...
void solve()
{
    ...
    cout << max({recur(0, 0), recur(0, 1),recur(0, 2)});
}
