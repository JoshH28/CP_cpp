...
void solve() {
    ll n, qn; cin >> n >> qn; ll arr[n+1], psum[n+1];
    for (ll q = 0; q < n; q++) {cin >> arr[q];} psum[0]=  0;
    for (ll q = 1; q <= n; q++) psum[q] = psum[q-1] + arr[q-1];
    ll currsum = 0, cnt = 5, t1;
    for (ll q = 0; q < qn; q++) {
        cin >> t1; currsum += t1;
        if (currsum >= psum[n]) {
            cout << n << "\n"; currsum = 0; continue;
        }
        ll dead = upper_bound(psum, psum+n+1, currsum)- psum - 1;
        cout << n - dead << "\n";        
    }
}
